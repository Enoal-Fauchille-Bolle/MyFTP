...
Test(command_help, basic_help_command)
{
    connection_t *connection;
    command_status_t command_status;
    command_t command = {.name = strdup("HELP"), .argc = 0, .argv = NULL};
    int read_fd;

    connection = setup_test_connection(&read_fd);
    cr_assert_not_null(connection);

    // Test unauthenticated help
    command_status = help_command(&command, connection);
    char buffer[1024] = {0};
