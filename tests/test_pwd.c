...
Test(command_pwd, basic_pwd_command)
{
    connection_t *connection;
    command_status_t command_status;
    command_t command = {.name = strdup("PWD"), .argc = 0, .argv = NULL};
    int read_fd;

    connection = setup_test_connection(&read_fd);
    cr_assert_not_null(connection);

    // Test unauthenticated pwd
    command_status = pwd_command(&command, connection);
    char buffer[1024] = {0};
    ...
    // Test authenticated pwd
    memset(buffer, 0, sizeof(buffer));
    connection->logged_in = true;
    command_status = pwd_command(&command, connection);
    fflush(connection->stream);  // Ensure data is written
    read(read_fd, buffer, sizeof(buffer));
    cr_assert_str_eq(buffer, "257 \"/tmp\"\r\n");
